            ...
            <section>
                <div class="card">
                    <div class="card-header">
                        <h6 class="card-title text-black">{{__('messages.Charity')}} | {{__('messages.champions_payments')}}</h6>
                        <hr>
                    </div>
                    ...
                    <div class="card-body">
                        <table class="table datatable-payments2 table-striped">
                            ...
                            <tbody>
                            <?php $i=1; ?>
                            @foreach($champion as $payment)
                                <tr>
                                    <td>{{$i}}</td>
                                    <td>
                                        {{$payment['name']!=""?$payment['name']." ".$payment['last_name']:''}}
                                        {{$payment['user_id']!=0?$payment['user']['people']['name']." ".$payment['user']['people']['family']:''}}</td>
                                    <td>{{$payment['phone']}}</td>
                                    <td>{{number_format($payment['amount'])}} {{__('messages.rial')}}</td>
                                    <td>
                                        @if($payment['created_at'])
                                            {{jdate("Y-m-d",strtotime($payment['created_at']))}}
                                        @endif
                                    </td>
                                    <td>{{$payment['gateway']['title']}}</td>
                                    <td>
                                        {{$payment['champion']['title']}}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="{{route('charity_champion_payments_show',['id'=>$payment['id']])}}"
