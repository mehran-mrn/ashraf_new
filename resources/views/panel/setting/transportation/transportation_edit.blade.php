<form action="{{route('setting_how_to_send_update')}}" method="post" id="frm_setting_how_to_send_add">
    <input type="hidden" value="{{$tran['id']}}" name="t_id">
    @csrf
    <div class="row">
        <div class="col-6">
            <div class="form-group">
                <label for="title">{{__('messages.title')}}</label>
                <input type="text" class="form-control" name="title" required id="title" value="{{$tran['title']}}">
            </div>
        </div>
        <div class="col-6">
            <div class="form-group">
                <label for="title">{{__('messages.send_time')}}</label>
                <input type="number" class="form-control" name="time" required id="time" value="{{$tran['time']}}">
            </div>
        </div>
        <div class="col-12">
        ...
        @foreach($province as $pro)
            ...
            $attribute = $pro->setting_transportation->find($tran['id']);
            if ($attribute) {
                $price = $attribute->pivot->cost;
            } else {
                $price = null;
            }
            ?>
            <div class="col-3">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control price" name="city_{{$pro['id']}}"
                           placeholder="{{__('messages.cost')}}" value="{{$price}}">
                </div>
            </div>
        @endforeach
            ...
            <div class="form-group mb-3 mb-md-2">
                ...
                <div class="custom-control custom-radio custom-control-inline">
                    <input type="radio" class="custom-control-input"
                           name="status"
                           id="custom_radio_inline_unchecked"
