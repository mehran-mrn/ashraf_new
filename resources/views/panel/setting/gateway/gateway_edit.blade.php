<form action="{{route('gateway_update',$info['id'])}}" method="post" id="frm_gateway_add">
    @csrf
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="form-group"><label for="name">{{__('messages.bank_name')}}</label>
                <select name="name" id="name" class="form-control">
                    ...
                    @foreach($banks as $bank)
                        <option value="{{$bank['id']}}" data-logo="{{$bank['logo']}}"
                            {{$bank['id']== $info['bank_id']?'selected':'' }} >{{$bank['name']}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="col-md-6">
            ...
            <div class="form-group form-group-feedback form-group-feedback-right">
                <input type="number" class="form-control text-right" maxlength="15" name="account_number"
                       id="account_number" value="{{$info['account_number']}}">
                <div class="form-control-feedback form-control-feedback-lg">
        ...
        <div class="col-md-6">
            ...
            <div class="form-group form-group-feedback form-group-feedback-right">
                <input type="number" class="form-control text-right" maxlength="24" name="account_sheba"
                       id="account_sheba" value="{{$info['account_sheba']}}">
                <div class="form-control-feedback form-control-feedback-lg">
                    IR
                </div>
            </div>
                ...
                <div class="p-3">
                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" class="custom-control-input" name="status" id="status" value="active"
                               checked data-fouc {{$info['status']=="active"?'checked':''}}>
    ...
    <div class="row">
            ...
            <div class="d-flex justify-content-center">
                <div class="p-3">
                    <div class="custom-control custom-checkbox custom-control-inline">
                        <input type="checkbox" class="custom-control-input" id="pay_online" name="pay_online" value="1"
                        {{$info['online']==1?'checked':''}}>
                        <label class="custom-control-label" for="pay_online">{{__('messages.online')}}</label>
                    </div>
                </div>
                <div class="p-3">
                    <div class="custom-control custom-checkbox custom-control-inline">
                        <input type="checkbox" class="custom-control-input" id="pay_cart" name="pay_cart" value="1"
                            {{$info['cart']==1?'checked':''}}>
                ...
                <div class="p-3">
                    <div class="custom-control custom-checkbox custom-control-inline">
                        <input type="checkbox" class="custom-control-input" id="pay_account" name="pay_account" value="1"
                            {{$info['account']==1?'checked':''}}>
        ...
        <div class="d-flex justify-content-center">
            <div class="p-3">
                <button type="submit" class="btn btn-primary btn-block px-5">{{__('messages.edit')}}</button>
            </div>
        </div>
    </div>
</form>
